...
// BİREBİR aynı desen — sadece bucket/eylem/etiket metinleri farklı.
export function ServiceImageUploader({ service }: { service: ServiceDetail }) {
  const uploadAction = uploadServiceImageAction.bind(null, service.id);
  ...
  const currentImageUrl = service.imagePath
    ? getPublicImageUrl("services", service.imagePath)
    : null;
  ...
  return (
    <div className="max-w-lg space-y-4 border-t border-neutral-300 pt-8">
      <h2 className="text-h5 font-semibold text-text">Hizmet Görseli</h2>
      ...
      {currentImageUrl && (
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <DeleteButton
              id={service.imagePath!}
              itemName={service.title}
              entityLabel="hizmet görseli"
              action={deleteServiceImageAction}
            />
          </div>
        </div>
      )}
      ...
      <form ref={formRef} action={formAction} className="space-y-3">
        <label className="block text-base text-text" htmlFor="service-image-input">
