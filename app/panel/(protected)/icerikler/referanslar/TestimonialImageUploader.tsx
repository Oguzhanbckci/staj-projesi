...
import {
  deleteTestimonialImageAction,
  uploadTestimonialImageAction,
...
// BİREBİR aynı desen.
export function TestimonialImageUploader({ testimonial }: { testimonial: TestimonialDetail }) {
  const uploadAction = uploadTestimonialImageAction.bind(null, testimonial.id);
  ...
  const currentImageUrl = testimonial.logoPath
    ? getPublicImageUrl("testimonials", testimonial.logoPath)
    : null;
  ...
  return (
    <div className="max-w-lg space-y-4 border-t border-neutral-300 pt-8">
      <h2 className="text-h5 font-semibold text-text">Referans Logosu</h2>
      ...
      {currentImageUrl && (
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <DeleteButton
              id={testimonial.logoPath!}
              itemName={testimonial.authorName}
              entityLabel="referans logosu"
              action={deleteTestimonialImageAction}
            />
          </div>
        </div>
      )}
      ...
      <form ref={formRef} action={formAction} className="space-y-3">
        <label className="block text-base text-text" htmlFor="testimonial-image-input">
